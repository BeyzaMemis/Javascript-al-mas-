...
function showQuetions(index){
    ...
    let que_tag = '<span>'+ questions[index].id + ". " + questions[index].title +'</span>';
    const options = questions[index].body.split('\n');
    let option_tag = '<div class="option"><span>'+ "A) " +options[0] +'</span></div>'
    + '<div class="option"><span>'+ "B) "+ options[1] +'</span></div>'
    + '<div class="option"><span>'+ "C) "+ options[2] +'</span></div>'
    + '<div class="option"><span>'+ "D) " + options[3] +'</span></div>';
...
function startTimerLine(time){
    ...
    function timer(){
        ...
        time_line.style.width = time / 2+ "px"; //increasing width of time_line with px by time value
        if(time > 1098){ //if time value is greater than 549
            clearInterval(counterLine); //clear counterLine
        }
    }
}
